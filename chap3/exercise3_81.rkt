...
(define (rand seed cmds)
  (define (rand-iter curr-rand cmds)
    (if (stream-empty? cmds)
        empty-stream
        (let ((cmd (stream-car cmds)))
          (cond ((eq? cmd 'generate)
                 (let ((next-rand (rand-update curr-rand)))
                   (stream-cons next-rand
                                (rand-iter next-rand (stream-cdr cmds)))))
                ((and (pair? cmd) (eq? (car cmd) 'reset))
                 (stream-cons (cdr cmd)
                              (rand-iter (cdr cmd) (stream-cdr cmds))))))))
  (rand-iter seed cmds))

(stream-print-first-n
 (rand 1 (stream 'generate (cons 'reset 2) 'generate (cons 'reset 54) 'generate))
 5)
