...
(define (stream-print-first-n s n)
  (if (= n 0)
      (display newline)
      (begin (display (stream-car (force s)))
             (display " ")
...
(define (monte-carlo experiment)
  (define (iter trials-passed trials-all)
    (if (experiment)
         (let ((new-trials-passed (+ trials-passed 1))
               (new-trials-all (+ trials-all 1)))
           (stream-cons
            (/ new-trials-passed new-trials-all)
            (iter new-trials-passed new-trials-all)))
         (let ((new-trials-all (+ trials-all 1)))
           (stream-cons
            (/ trials-passed new-trials-all)
            (iter trials-passed new-trials-all)))))
  (iter 0 0))
...
(define (estimate-integral p x1 x2 y1 y2)
  ...
  (stream-map (lambda (x) (* x (- x2 x1) (- y2 y1)))
              (monte-carlo integral-experiment)))

(define (is-inside-unit-circle? x y)
  (<= (+ (* x x) (* y y)) 1))

(stream-print-first-n
 (estimate-integral is-inside-unit-circle? (- 1) 1 (- 1) 1)
 100000)
