...
;(stream-print-first-n integers-pairs-not-div-by-235-ordered 40)
...
(define (pair-cube-sum x)
  (+ (cube (car x))
     (cube (cadr x))))

(define pairs-by-sum-of-square
  (weighted-pairs integers
                  integers
                  pair-cube-sum))

(define (find-pairs-with-eq-sum s)
    (let ((first (stream-car s))
      ...
      (display second)
      (display " = ")
      (display (pair-cube-sum first))
      (newline)
      (if (= (pair-cube-sum first) (pair-cube-sum second))
          (stream-cons first (find-pairs-with-eq-sum (stream-cdr (stream-cdr s))))
          (find-pairs-with-eq-sum (stream-cdr s)))))

(define ramujan-numbers
  (stream-map pair-cube-sum
              (find-pairs-with-eq-sum pairs-by-sum-of-square)))