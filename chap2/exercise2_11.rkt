...
; Checks if x is positive (or zero) number.
;
; Arguments:
; x - number
;
; Returns:
; #t when x >=0, #f otherwise
(define (pos? x)
  (>= x 0))

; Checks if x is negative number.
;
; Arguments:
; x - number
;
...
; mul-interval with different implementation(by 9 separate cases)
(define (mul-interval_exp x y)
  (let ((xlow (lower-bound x))
    ...
    (cond ((and (pos? xlow)
                ...
                (pos? ylow)
                (pos? yupp))
           (make-interval (* xlow ylow) (* xupp yupp)))
          ((and (neg? xlow)
                (pos? xupp)
                (pos? ylow)
                (pos? yupp))
           (make-interval (* xlow yupp) (* xupp yupp)))
          ((and (neg? xlow)
                (neg? xupp)
                (pos? ylow)
                (pos? yupp))
           (make-interval (* xlow yupp) (* xupp ylow)))
          ((and (pos? xlow)
                (pos? xupp)
                (neg? ylow)
                (pos? yupp))
           (make-interval (* xupp ylow) (* xupp yupp)))
          ((and (neg? xlow)
                (pos? xupp)
                (neg? ylow)
                (pos? yupp))
           (make-interval (min (* xlow yupp) (* xupp ylow))
                          (max (* xlow ylow) (* xupp yupp))))
          ((and (neg? xlow)
                (neg? xupp)
                (neg? ylow)
                (pos? yupp))
           (make-interval (* xlow yupp) (* xlow ylow)))
          ((and (neg? xlow)
                (neg? xupp)
                (neg? ylow)
                (neg? yupp))
           (make-interval (* xupp yupp) (* xlow ylow)))
          ((and (pos? xlow)
                (pos? xupp)
                (neg? ylow)
                (neg? yupp))
           (make-interval (* xupp ylow) (* xlow yupp)))
          ((and (neg? xlow)
                (pos? xupp)
                (neg? ylow)
                (neg? yupp))
