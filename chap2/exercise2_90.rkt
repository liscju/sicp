  ...
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags))
      ...
      (if proc
          ...
          (if (or (<= (length args) 1) (same-all-elements? type-tags))
         ...
         (let ((arg1 (car args))
           ...
           (let ((arg1type (type-tag arg1))
             ...
             (cond ((is-subtype? arg1 arg2type)
                   ...
                   (else (error "Arguments are not subtypes"))))))
         (error
          "length more than 2")))
...
(define (install-rational-package)
  ...
  (put 'sine '(rational)
       (lambda (x) (sin (round (/ (numer x) (denom x))))))
  (put 'cosine '(rational)
       (lambda (x) (cos (round (/ (numer x) (denom x))))))
  (put 'atane '(rational rational)
       (lambda (x y) (atan (round (/ (numer x) (denom x)))
                           (round (/ (numer y) (denom y))))))
  (put 'rounde '(rational)
...
(define (install-thick-polynomial-package)
  ...
  ; terms (thick polynomial) implementation
  (define (adjoin-term term term-list)
    (define (iterate order coeff terms)
      (cond ((and (= order 0) (null? terms))
             (list coeff))
            ((and (= order 0) (not (null? terms)))
             (cons (+ coeff (car terms)) (cdr terms)))
            ((not (null? terms))
             (cons (car terms) (iterate (dec order) coeff (cdr terms))))
            (else
             (cons 0 (iterate (dec order) coeff nil)))))
    (let ((order (car term))
          (coeff (cadr term)))
      (reverse (iterate order coeff (reverse term-list)))))
  (define (the-empty-termlist) '())
  (define (first-term term-list)
    (list (dec (length term-list)) (car term-list)))
  (define (rest-terms term-list) (cdr term-list))
  (define (empty-termlist? term-list) (null? term-list))
  ...
  (define (tag p) (attach-tag 'polynomial-thick p))
  (put 'add '(polynomial-thick polynomial-thick)
       (lambda (p1 p2) (tag (add-poly p1 p2))))
  (put 'sub '(polynomial-thick polynomial-thick)
       (lambda (p1 p2) (tag (add-poly p1 (opposite p2)))))
  (put 'mul '(polynomial-thick polynomial-thick)
       (lambda (p1 p2) (tag (mul-poly p1 p2))))
  (put 'opposite '(polynomial-thick)
       (lambda (p) (make-polynomial (variable p)
                                    (mul-term-by-all-terms
                                     (make-term 0 (- 1))
                                     (term-list p)))))
  (put 'make 'polynomial-thick
       (lambda (var terms) (tag (make-poly var terms))))
  (put 'project '(polynomial-thick)
       (lambda (p) (tag p)))
  (put 'raise '(polynomial-thick)
       (lambda (p) (tag p)))
  'done)

(define (make-polynomial-thick var terms)
  ((get 'make 'polynomial-thick) var terms))

(define (install-thin-polynomial-package)
  (define (make-poly variable term-list)
    (cons variable term-list))
  (define (variable p) (car p))
  (define (term-list p) (cdr p))
  ...
  (define (tag p) (attach-tag 'polynomial-thin p))
  (put 'add '(polynomial-thin polynomial-thin)
       (lambda (p1 p2) (tag (add-poly p1 p2))))
  (put 'sub '(polynomial-thin polynomial-thin)
       (lambda (p1 p2) (tag (add-poly p1 (opposite p2)))))
  (put 'mul '(polynomial-thin polynomial-thin)
       (lambda (p1 p2) (tag (mul-poly p1 p2))))
  (put 'opposite '(polynomial-thin)
       (lambda (p) (make-polynomial (variable p)
                                    (mul-term-by-all-terms
                                     (make-term 0 (- 1))
                                     (term-list p)))))
  (put 'make 'polynomial-thin
       (lambda (var terms) (tag (make-poly var terms))))
  (put 'project '(polynomial-thin)
       (lambda (p) (tag p)))
  (put 'raise '(polynomial-thin)
       (lambda (p) (tag p)))
  'done)
...
(install-thick-polynomial-package)
(install-thin-polynomial-package)
